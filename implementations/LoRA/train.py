...
def build_model(mode, r=8, alpha=16, target_modules=("q_lin", "v_lin")):
    ...
    if mode == "full":
        ...
        model = inject_lora(model, target_modules=target_modules, r=r, alpha=alpha)
        freeze_all_but_lora_and_head(model)
    return model


def train(mode, epochs=3, lr=None, subset_size=None, r=8, alpha=16, target_modules=("q_lin", "v_lin"),log_path="results/metrics.csv"):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    train_dl, val_dl = get_dataloaders(subset_size=subset_size)
    model = build_model(mode, r=r, alpha=alpha, target_modules=target_modules).to(device)

    trainable, total, pct = count_trainable_params(model)
    lr = lr or (2e-5 if mode == "full" else 1e-3)
    ...
    result = dict(mode=mode, r=r if mode == "lora" else None,
                  target_modules="+".join(target_modules) if mode == "lora" else None,
